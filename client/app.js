...
function getIo() {
    const w = window;
    if (!w.io)
        throw new Error("Socket.IO not loaded. Check index.html script order.");
    return w.io;
}
...
function saveRoomState(roomId, playerIndex, plateCount) {
...
async function createRoom(plateCount) {
...
async function leaveRoom(roomId, playerIndex) {
    ...
    const data = (await res.json());
    if (!data.ok)
        throw new Error(data.error ?? "방 나가기에 실패했습니다.");
}
...
function renderLobby() {
  ...
  `;
    ...
    createRoomBtn.addEventListener("click", async () => {
        ...
        const selectEl = document.getElementById("plateCountSelect");
...
function renderGame() {
  ...
  `;
    ...
    let gameSubPhase = "round";
    /** 메인 단계에서 선수가 선택한 접시(뚜껑) 인덱스 최대 2개 */
    const selectedPlatesForMain = [];
    /** 메인 단계 하위: 두 접시 선택 중 | 토큰 하나 놓을 접시 선택 중 */
    let mainSubPhase = "selectTwoPlates";
    /** 상대가 두 접시 선택 시 받은 [plateA, plateB] (상대가 토큰 놓은 뒤 뚜껑 닫을 때 사용) */
    let opponentSelectedPlatesForMain = null;
    ...
    const onGameStart = (data) => {
        const payload = data;
        if (payload?.roomId !== roomState.roomId)
            return;
        gamePhase = "playing";
    ...
    function doLeaveRoom(options) {
        ...
        if (gameSocket)
            gameSocket.off("room:playerLeft", onPlayerLeft);
        if (gameSocket)
            gameSocket.off("game:start", onGameStart);
        if (gameSocket)
            gameSocket.off("game:plateClick", onPlateClickFromServer);
        if (gameSocket)
            gameSocket.off("game:mainTwoPlatesSelected");
        if (gameSocket)
            gameSocket.off("game:mainTokenPlaced");
        if (gameSocket)
            gameSocket.off("game:mainWrongAnswerDone");
        if (gameSocket)
            gameSocket.off("game:mainTimeOver");
        if (gameSocket)
            gameSocket.off("game:turnSwitch", onTurnSwitch);
        if (gameSocket)
            gameSocket.off("game:gameOver", onGameOver);
        if (gameSocket)
            gameSocket.off("game:roomClosed", onGameRoomClosed);
        ...
        clearRoomState();
        if (options.showOpponentLeftAlert)
            alert("상대가 방을 나갔습니다.");
        renderLobby();
    ...
    const onPlayerLeft = (data) => {
        ...
        if (payload?.roomId !== roomState.roomId)
            return;
        if (payload?.playerIndex === roomState.playerIndex)
            return;
    ...
    function updateTokenLabels() {
        ...
        if (p1El)
            p1El.textContent = String(Math.max(0, roundRemainingP1));
        if (p2El)
            p2El.textContent = String(Math.max(0, roundRemainingP2));
    }
    /** 클릭 전: round 단계는 토큰 N개 문구, 메인 단계는 확인할 두 접시 문구 */
    function updateCenterMessageForWaiting() {
        const msgEl = document.getElementById("gameCenterMessage");
        if (!msgEl || gamePhase !== "playing")
            return;
        ...
        if (transitionToMainDone)
            return;
        const rawRound = currentTurn === roomState.playerIndex ? myClickCount + 1 : lastOpponentRound + 1;
    ...
    function setPlateTokens(plateIndex, counts) {
        const container = document.querySelector(`#gamePlateRing .game-plate[data-plate-index="${plateIndex}"] .game-plate-tokens`);
        if (!container)
            return;
        container.innerHTML = "";
        const c0 = counts[0] ?? 0;
    ...
    function setCenterMessagePlaced(plateNum, round) {
        if (transitionToMainDone)
            return;
        const msgEl = document.getElementById("gameCenterMessage");
        if (!msgEl)
    ...
    function doTransitionToMainGame() {
        if (transitionToMainDone)
            return;
        transitionToMainDone = true;
        const msgEl = document.getElementById("gameCenterMessage");
        if (msgEl)
            msgEl.textContent = "빈 접시의 뚜껑을 덮습니다";
        const uncovered = Array.from({ length: plateCount }, (_, i) => i).filter((i) => !usedPlates.has(i));
        const two = uncovered.slice(0, 2);
        two.forEach((i) => {
            usedPlates.add(i);
            const lid = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${i}"]`);
            if (lid && !lid.classList.contains("game-plate-lid--covered")) {
                lid.classList.add("game-plate-lid--covered");
            }
            const plateEl = document.querySelector(`#gamePlateRing .game-plate[data-plate-index="${i}"]`);
            if (plateEl)
                plateEl.classList.add("game-plate--used");
        });
        inputBlocked = true;
        setTimeout(() => {
            if (msgEl)
                msgEl.textContent = "이제 게임을 시작합니다";
            const timerEl = document.getElementById("gameTimer");
            if (timerEl)
                timerEl.classList.add("game-timer--hidden");
            setTimeout(() => {
                if (timerEl)
                    timerEl.classList.remove("game-timer--hidden");
                inputBlocked = false;
                gameSubPhase = "main";
    ...
    function toggleMainPlateSelection(plateIndex) {
        if (gameEnded)
            return;
        ...
        if (selectedPlatesForMain.length === 2) {
            ...
            const lidA = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${a}"]`);
            const lidB = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${b}"]`);
            if (lidA)
                lidA.classList.add("game-plate-lid--uncover");
            if (lidB)
                lidB.classList.add("game-plate-lid--uncover");
            ...
            if (!same && msgEl)
                msgEl.textContent = "틀렸습니다";
            setTimeout(() => {
                if (same) {
                    mainSubPhase = "placeToken";
                    // 이 플레이어가 토큰1개 남아있는 상태라면 승리로 게임오버
                    if ((roomState.playerIndex === 0 && heldTokensP1 === 1) ||
                    ...
                    if (msgEl)
                        msgEl.textContent = "토큰 1개를 놓을 접시를 선택해주세요";
                    [a, b].forEach((i) => {
                        const plateEl = document.querySelector(`#gamePlateRing .game-plate[data-plate-index="${i}"]`);
                        if (plateEl)
                            plateEl.classList.add("game-plate--placeable");
                    });
                }
                else {
                    if (msgEl)
                        msgEl.textContent = "페널티로 토큰 1개를 받습니다";
                    ...
                    const mySlot = document.querySelector(`.player-slot[data-slot="${roomState.playerIndex}"] .player-tokens`);
    ...
    function placeTokenOnPlate(plateIndex) {
        if (gameEnded)
            return;
        if (mainSubPhase !== "placeToken" || selectedPlatesForMain.length !== 2)
            return;
        if (!selectedPlatesForMain.includes(plateIndex))
            return;
        inputBlocked = true;
        const [a, b] = [selectedPlatesForMain[0], selectedPlatesForMain[1]];
        ...
        const msgEl = document.getElementById("gameCenterMessage");
        if (msgEl)
            msgEl.innerHTML = `<span class="game-center-highlight">${plateNum}</span>번 접시에 토큰 1개를 더 놓았습니다`;
        const mySlot = document.querySelector(`.player-slot[data-slot="${roomState.playerIndex}"] .player-tokens`);
    ...
    function onPlateClick(index) {
        if (gameEnded)
            return;
        if (gamePhase !== "playing" || currentTurn !== roomState.playerIndex)
            return;
        if (inputBlocked || usedPlates.has(index))
            return;
        ...
        const lid = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${index}"]`);
        if (lid && !lid.classList.contains("game-plate-lid--covered")) {
            lid.classList.add("game-plate-lid--covered");
        }
        const plateEl = document.querySelector(`#gamePlateRing .game-plate[data-plate-index="${index}"]`);
        if (plateEl)
            plateEl.classList.add("game-plate--used");
        ...
        ringEl?.dispatchEvent(new CustomEvent("plateclick", { detail: { index }, bubbles: true }));
        setTimeout(() => {
    ...
    function doMainTimeOverPenalty() {
        inputBlocked = true;
        ...
        const msgEl = document.getElementById("gameCenterMessage");
        if (msgEl)
            msgEl.textContent = "시간 오버 페널티로 토큰1개를 받습니다";
        const mySlot = document.querySelector(`.player-slot[data-slot="${currentTurn}"] .player-tokens`);
    ...
    function startTimer() {
        ...
        const duration = gameSubPhase === "main" ? MAIN_GAME_TIMER_SECONDS : TIMER_DEFAULT_SECONDS;
        timerSecondsLeft = duration;
        const timerEl = document.getElementById("gameTimer");
        if (timerEl)
            timerEl.textContent = formatTimer(timerSecondsLeft);
        timerIntervalId = setInterval(() => {
            timerSecondsLeft = Math.max(0, timerSecondsLeft - 1);
            if (timerEl)
                timerEl.textContent = formatTimer(timerSecondsLeft);
            if (timerSecondsLeft <= 0 && timerIntervalId) {
                if (gameSubPhase === "round" &&
                    currentTurn === roomState.playerIndex &&
                    gamePhase === "playing" &&
                    !inputBlocked) {
    ...
    const onPlateClickFromServer = (data) => {
        ...
        const lid = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${plateIndex}"]`);
        if (lid && !lid.classList.contains("game-plate-lid--covered")) {
            lid.classList.add("game-plate-lid--covered");
        }
        const plateEl = document.querySelector(`#gamePlateRing .game-plate[data-plate-index="${plateIndex}"]`);
        if (plateEl)
            plateEl.classList.add("game-plate--used");
        const cur = plateTokensByPlayer[plateIndex] ?? { 0: 0, 1: 0 };
        const p = (payload.playerIndex ?? 0);
        const next = {
            0: cur[0] + (p === 0 ? round : 0),
            1: cur[1] + (p === 1 ? round : 0),
        };
    ...
    const onMainTwoPlatesSelected = (data) => {
        const payload = data;
        if (payload?.roomId !== roomState.roomId)
            return;
        if (payload.playerIndex === roomState.playerIndex)
        ...
        const lidA = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${plateA}"]`);
        const lidB = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${plateB}"]`);
        if (lidA)
            lidA.classList.add("game-plate-lid--uncover");
        if (lidB)
            lidB.classList.add("game-plate-lid--uncover");
        ...
        if (payload.correct && msgEl)
            msgEl.textContent = "상대가 정답을 맞췄습니다";
        if (!payload.correct && msgEl)
            msgEl.textContent = "상대가 틀렸습니다";
        const LID_UNCOVER_MS = 1500;
        setTimeout(() => {
            if (payload.correct && msgEl)
                msgEl.textContent = "상대가 토큰 1개를 놓을 접시를 선택중입니다";
            if (!payload.correct && msgEl) {
                msgEl.textContent = "상대방이 페널티로 토큰 1개를 받았습니다";
                const who = (payload.playerIndex ?? 0);
                if (who === 0)
                    heldTokensP1 += 1;
                else
                    heldTokensP2 += 1;
                updateTokenLabels();
                const opponentSlot = document.querySelector(`.player-slot[data-slot="${who}"] .player-tokens`);
    ...
    const onMainTimeOver = (data) => {
        ...
        const msgEl = document.getElementById("gameCenterMessage");
        if (msgEl)
            msgEl.textContent = "상대방이 시간오버 페널티로 토큰1개를 받습니다";
        const opponentSlot = document.querySelector(`.player-slot[data-slot="${who}"] .player-tokens`);
        const tokenClass = who === 0 ? "player-token player-token--p1" : "player-token player-token--p2";
        const penaltyToken = document.createElement("span");
        penaltyToken.className = tokenClass;
    ...
    const onGameOver = (data) => {
        ...
        const msgEl = document.getElementById("gameCenterMessage");
        if (msgEl)
            msgEl.textContent = "GameOver";
        setTimeout(() => {
            if (msgEl)
                msgEl.textContent = "잠시 후 종료됩니다";
            setTimeout(() => {
    ...
    const onMainWrongAnswerDone = (data) => {
        const payload = data;
        if (payload?.roomId !== roomState.roomId)
            return;
        if (payload.playerIndex === roomState.playerIndex)
        ...
        const lidA = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${pa}"]`);
        const lidB = document.querySelector(`#gamePlateRing .game-plate-lid[data-plate-index="${pb}"]`);
        const recoverLid = (lid) => {
            if (!lid)
                return;
            lid.classList.remove("game-plate-lid--uncover");
    ...
    const onMainTokenPlaced = (data) => {
        ...
        updateTokenLabels();
        const opponentSlot = document.querySelector(`.player-slot[data-slot="${payload.playerIndex}"] .player-tokens`);
    ...
    function renderGameArea() {
        ...
        ${readySection}
      `;
            const readyBtn = content.querySelector("#readyBtn");
            if (readyBtn) {
      ...
      `;
                ...
                if (canActMain) {
                    ...
                    if (lid) {
                        ...
                        if (indexStr !== null) {
                            const index = parseInt(indexStr, 10);
                            if (!Number.isNaN(index))
                                toggleMainPlateSelection(index);
                        }
                        return;
                    }
                }
            ...
            ringEl?.addEventListener("keydown", (e) => {
                ...
                if (ke.key !== "Enter" && ke.key !== " ")
                    return;
                const plate = e.target.closest(".game-plate");
                if (!plate)
                    return;
                ke.preventDefault();
                const indexStr = plate.getAttribute("data-plate-index");
    ...
    if (gameWrap) {
        gameWrap.addEventListener("click", (e) => {
            ...
            if (currentTurn === roomState.playerIndex)
                return;
            if (target.closest("#backToLobbyBtn"))
                return;
    ...
    function updateGameUI(room) {
        ...
        if (slots && slots.length >= 2) {
            ...
            slots.forEach((slot, i) => {
                ...
                if (nameEl)
                    nameEl.textContent = names[i] ?? "대기 중";
                if (badgeEl) {
                    badgeEl.hidden = i !== myIndex;
                }
